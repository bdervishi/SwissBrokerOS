...
import { Check, Star, Plus, Edit2, Users, Crown, Zap, ShieldCheck, CreditCard, Layout as LayoutIcon, BrainCircuit, Palette, ShoppingBag, CheckCircle, Loader2, Server, Lock, Building2 } from 'lucide-react';
...
export const SaaSPlans: React.FC = () => {
    ...
    const handleSuccessAction = () => {
        ...
        if (purchasingAddon?.id === 'addon_website') {
            navigate('/web-engine');
        } else if (purchasingAddon?.id === 'addon_corporate_suite') {
            navigate('/clients');
        }
    };
                            ...
                            <div key={addon.id} className={`p-6 rounded-xl border transition-all ${isActive ? 'bg-emerald-50/50 border-emerald-200 dark:bg-emerald-900/10 dark:border-emerald-900/30' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                                ...
                                {isActive ? (
                                    <Button 
                                        ...
                                        onClick={() => {
                                            if(addon.id === 'addon_website') navigate('/web-engine');
                                            if(addon.id === 'addon_corporate_suite') navigate('/clients');
                                        }}
                                    >
                                        Modul öffnen
                                    </Button>
                    ...
                    {purchaseStep === 'SUCCESS' && purchasingAddon && (
                        <div className="text-center space-y-6">
                            ...
                            <Button className="w-full" onClick={handleSuccessAction}>
                                {purchasingAddon.id === 'addon_website' ? 'Web-Engine starten' : (purchasingAddon.id === 'addon_corporate_suite' ? 'Firmen-Kunden öffnen' : 'Schliessen')}
                            </Button>
                        </div>
                    )}
                </div>
